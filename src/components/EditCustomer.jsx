...
import { getUserFunction } from "../api/api";
import dayjs from "dayjs";

function EditCustomer({ setShowModal , userid }) {
    const {editUser,getUsersList} = useUser();
  ...
  const getUser = async () => {
      try {
        const response = await getUserFunction(userid, token);
        setFormData({
          name: response.data.name,
          address: response.data.address,
          email: response.data.email,
          phone: response.data.phone,
          bdate: dayjs(response.data.birthDate).format('YYYY-MM-DD'),
        })
